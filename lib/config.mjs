...
export function loadConfig(env = process.env) {
  // Vercel's application bundle is read-only; /tmp is writable but instance-local.
  if (env.VERCEL === '1') env = { AEGIS_DB_PATH: '/tmp/aegis.sqlite', HOST: '0.0.0.0', TRUST_PROXY: 'true', ...env };
  const config = {};
